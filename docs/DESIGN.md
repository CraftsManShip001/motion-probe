...
- **터치 경계**: 프로브가 손가락이 닿아 있던 구간(`touches`)을 함께 기록한다. 손가락이 닿거나 떨어지는 순간 새 동작(누름 애니메이션, release·fling)이 시작되므로 그 프레임에서 구간을 자른다. 손가락이 10pt 이상 움직인 구간의 변화는 `gesture`(손가락을 따라감)로 표시하고 곡선 피팅·jump·stall 판정을 하지 않는다. 손을 뗀 뒤의 구간은 일반 애니메이션으로 분석한다.
- **투명도 세 가지**: 자기 `opacity`, 조상이 페이드한 `inheritedOpacity`(= effective ÷ own, 화면·카드 전환), 뷰 안에 그려지는 내용의 `contentOpacity`를 따로 본다. 이미지 라이브러리는 태그한 뷰가 아니라 안쪽 이미지를 페이드하기 때문이다(expo-image: Android는 자식 ImageView alpha, iOS는 cross-dissolve 트랜지션). 내용은 자식의 이미지·텍스트·배경을 겹친 레이어처럼 합성(1 − Π(1 − α))하고, 자기 배경은 제외한다(플레이스홀더 색 위로 이미지가 페이드 인하는 경우). Fabric은 stacking context를 만들지 않는 뷰의 자식을 그 뷰의 **뒤쪽 형제**로 마운트하므로, 대상 영역 안에 있고 나중에 그려지는 형제도 내용으로 친다.
...
- **Android 창 애니메이션**: 코어 `<Modal>`의 슬라이드, 액티비티 전환 같은 창(window) 애니메이션은 시스템 컴포지터에서 돌아서 앱 안의 프로브에는 보이지 않는다. 내용의 레이아웃은 기록된다.
- **디스플레이 링크 순서**: 우리 CADisplayLink와 애니메이션 드라이버의 콜백 순서가 정해져 있지 않아 최대 1프레임 오차가 생길 수 있다. 첫 프레임 진행률이 0에 가까운 in-out 곡선은 "늦게 시작한 곡선"과 "천천히 시작하는 곡선"을 구분할 수 없어서 길이가 1프레임 길게 나올 수 있다.
- **내용 불투명도의 근사**: 조각들을 겹친 레이어처럼 합성하므로 겹치지 않는 내용은 실제보다 불투명하게 나온다. iOS cross-dissolve는 트랜지션 직전 프레임의 내용에서 섞는다고 보고 계산한다. 텍스트 글리프 단위나 그림자는 보지 않는다.
- **터치 관찰 범위**: iOS는 앱의 모든 창에 관찰용 제스처 인식기(인식하지 않음)를 붙이고, Android는 액티비티 창의 `Window.Callback`을 감싼다. Android의 Dialog·Modal 창에서 일어난 터치는 보지 못한다.
